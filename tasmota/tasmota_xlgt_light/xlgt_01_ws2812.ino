...
#endif // USE_DERG_RGB  kIncandescent, 2,
{
  kRgb, 3,
  kChristmas, 2,
  kHanukkah, 2,
...
void SetMisanPartlist(uint8_t partlistIdx) {
  switch(partlistIdx) {
    ...
    case 4: { // food
      uint8_t new_parts[] = { 0x02, 0x00, 0x00, 0x6C, 0x00, 0x6B, 0x00, 0x01, 0x00 };
      std::copy(new_parts, new_parts+sizeof(new_parts)/sizeof(new_parts[0]), misan);
      break;
    }
  }
...
void SetMisanTemplate(uint8_t mode) {
  ...
  switch (mode) {
    ...
    case 2: { // Food Strip
      uint8_t new_segment[] = { 0x06, 0x00, 0x52, 0x03, 0x03, 0x06, 0x00, 0x09, 0x03, 0x03, 0x05, 0x02, 0x30, 0x80, 0xff, 0x00, 0x00 };
      std::copy(new_segment, new_segment+sizeof(new_segment)/sizeof(new_segment[0]), segments);
      break;
    }
